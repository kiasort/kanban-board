<template>
  <form class="card-form" @submit.prevent="handleSubmit">
    <div class="field">
      ...
      <input v-model="title" type="text" placeholder="Что нужно сделать?" required autofocus />
    </div>
...
const emit = defineEmits<{
  submit: [data: { title: string; description: string; priority: 'low' | 'medium' | 'high'; columnId: number }]
  cancel: []
}>()

const title = ref(props.card.title)
const description = ref(props.card.description)
const priority = ref<'low' | 'medium' | 'high'>(props.card.priority)
const columnId = ref(props.card.columnId)
